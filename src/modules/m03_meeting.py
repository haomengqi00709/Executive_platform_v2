...
def _attendees_from_call_records(rec: dict, app_token: str) -> list[dict]:
    ...
    # Same time-anchor strategy as _attendees_from_calendar: createdDateTime
    # is OneDrive UTC (reliable), filename embeds the recorder's LOCAL time
    # and is only a fallback. Treating filename as UTC was the long-standing
    # bug fixed in 865eef9 for the calendar path; mirror that fix here.
    rec_dt = None
    created = rec.get("createdDateTime") or (rec.get("remoteItem") or {}).get("createdDateTime")
    if created:
        try:
            rec_dt = datetime.fromisoformat(created.replace("Z", "+00:00"))
        except ValueError:
            pass
    if not rec_dt:
        name = rec.get("name", "")
...
def _attendees_from_calendar(rec: dict, graph: GraphClient) -> list[dict]:
    ...
    # Subject validation: the recording filename is generated by Teams from
    # the meeting subject ("Sync Daniel Jason Max-20260610_103045-Meeting
    # Recording.mp4"). Match it against each candidate event's subject before
    # trusting that event's attendee list. Pre-fix, this function picked the
    # event closest in time to the recording regardless of subject, and on
    # back-to-back meetings or ad-hoc Teams calls (no calendar invite) it
    # silently grabbed the wrong meeting's attendees — Daniel's 2026-06-10
    # incident where the "Sync Daniel Jason Max" summary got mailed to the
    # circ.earth attendees from an unrelated earlier meeting.
    def _normalize(s: str) -> str:
        return re.sub(r"\s+", " ", re.sub(r"[^a-z0-9 ]+", " ", (s or "").lower())).strip()

    recording_subject = _parse_title_from_header("", rec.get("name", ""))
    recording_norm    = _normalize(recording_subject)

    def _subject_matches(ev) -> bool:
        subj = _normalize(ev.get("subject") or "")
        if not subj or not recording_norm:
            return False
        # Bi-directional containment handles Teams filename truncation in
        # either direction and minor punctuation/casing differences.
        return subj in recording_norm or recording_norm in subj

    candidates = [e for e in events if _subject_matches(e)]
    print(f"[m03 calendar] window={len(events)} subject_match={len(candidates)} "
          f"recording_subject={recording_subject!r}", flush=True)
    if not candidates:
        # Deliberately do NOT fall through to "closest event in time".
        # Better to save the draft with no recipients (caller handles this)
        # than to silently address someone else's meeting attendees.
        return []

    # Within subject-matched candidates, prefer the one whose END is closest
    # to the recording's createdDateTime — Teams writes the OneDrive file
    # when upload finishes (post-meeting), so end-time is the right anchor.
    def _dt_end(ev):
        try:
            end = datetime.fromisoformat(ev["end"]["dateTime"].replace("Z", "+00:00"))
            return abs((meeting_dt - end).total_seconds())
        except Exception:
            return 9999999

    best = min(candidates, key=_dt_end)
    print(f"[m03 calendar] picked subject={best.get('subject')!r} "
          f"start={best.get('start',{}).get('dateTime')}", flush=True)
...
def run(
    ...
    # ── Path 2: OneDrive recordings ────────────────────────
        ...
        for rec in mp4_files:
            ...
            # Two-step attendee resolution:
            #   1. Call Records API (/communications/callRecords) — most
            #      authoritative: directly tied to the specific Teams call,
            #      no time-window guessing. Requires CallRecords.Read.All app
            #      permission; returns [] cleanly if not granted.
            #   2. Subject-validated calendar match — falls back here when
            #      Call Records is empty (no permission, ad-hoc call not in
            #      the tenant's recent call records, etc.).
            #
            # Pre-2026-06-10 this was just the calendar call with no subject
            # check, which silently grabbed the wrong meeting's attendees on
            # back-to-back meetings (the original "Sync Daniel Jason Max"
            # summary → circ.earth recipients incident).
            ...
            if not attendees_info:
                _p("  No reliable attendee match (CallRecords + Calendar both empty "
                   "or subject mismatch) — draft will be saved without recipients")
