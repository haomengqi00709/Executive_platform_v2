...
def test_migration_imports_only_receipts(tmp_path):
    """Expenses are reimbursable receipts only: the xlsx receipt migrates; invoice/contract rows
    in results/expenses.json are NOT imported."""
    _seed_xlsx(tmp_path, [_receipt_xlsx_row()])
    _seed_results(tmp_path, [_invoice_item(), _contract_item()])
    items = es.load_expenses(tmp_path)
    assert {it["document_type"] for it in items} == {"receipt"}, [i["document_type"] for i in items]
    assert items[0]["vendor"] == "Starbucks"
    assert es.get_migration_status(tmp_path)["verdict"] == "lossless"


def test_migration_is_idempotent(tmp_path):
    _seed_xlsx(tmp_path, [_receipt_xlsx_row()])
    _seed_results(tmp_path, [_invoice_item()])   # invoice ignored — receipts only
...
def test_projection_is_receipts_only(tmp_path):
    """receipt → xlsx + json; invoice/contract are purged by the projection (receipts only)."""
    es.upsert_expense(tmp_path, es._row_from_excel(_receipt_xlsx_row()))
    es.upsert_expense(tmp_path, _invoice_item())      # purged
    es.upsert_expense(tmp_path, _contract_item())     # purged
    ...
    assert {it["document_type"] for it in res["items"]} == {"receipt"}
    assert {it["document_type"] for it in es.load_expenses(tmp_path)} == {"receipt"}


def test_invoice_is_purged_not_stored(tmp_path):
    es.upsert_expense(tmp_path, _invoice_item())      # projection purges non-receipts
...
def test_update_expense_fields_preserves_siblings(tmp_path):
    eid = es.upsert_expense(tmp_path, es._row_from_excel(_receipt_xlsx_row()))
    es.update_expense_fields(tmp_path, eid, {"amount": 999})
    it = es.load_expenses(tmp_path)[0]
    assert it["amount"] == 999
    assert it["vendor"] == "Starbucks" and it["category"] == "Meals"   # untouched
